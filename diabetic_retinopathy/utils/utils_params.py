...
def gen_run_folder(path_model_id=''):
    ...
    run_paths['path_plot'] = os.path.join(run_paths['path_model_id'], 'plot')

    # Create folders
    for k, v in run_paths.items():
        if any([x in k for x in ['path_model', 'path_ckpts', 'path_plot']]):
            if not os.path.exists(v):
                os.makedirs(v, exist_ok=True)
                if k == 'path_plot':
                    if not os.path.exists(v + '/test'):
                        os.makedirs(v + '/test', exist_ok=True)
                    if not os.path.exists(v + '/train'):
